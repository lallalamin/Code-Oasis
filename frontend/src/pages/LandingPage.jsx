...
const LeaderBoard = () => {
  ...
  const getRelativeRanks = () => {
    if (!currentUserRank || leaderboard.length === 0) return [];

    const startIndex = Math.max(0, currentUserRank - 3); // 2 ranks above
    const endIndex = Math.min(leaderboard.length, currentUserRank + 2); // 2 ranks below

    let result = leaderboard.slice(startIndex, endIndex);

    // Adjust to always have 5 users if possible
    while (result.length < 5 && startIndex > 0) {
      result = [leaderboard[startIndex - 1], ...result];
    }

    while (result.length < 5 && endIndex < leaderboard.length) {
      result = [...result, leaderboard[endIndex]];
    }

    return result;
  };

  const relativeRanks = getRelativeRanks();
